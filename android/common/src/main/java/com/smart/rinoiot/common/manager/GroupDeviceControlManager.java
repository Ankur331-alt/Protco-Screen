...
import com.smart.rinoiot.common.bean.GroupPanelBean;
 ...
 * 群组设备面板数据处理
 */
public class GroupDeviceControlManager {
    private static GroupDeviceControlManager instance;

    private GroupDeviceControlManager() {
    }

    public static GroupDeviceControlManager getInstance() {
        if (instance == null) {
            instance = new GroupDeviceControlManager();
        }
        return instance;
    }
            ...
            public void onSuccess(PanelInfo data) {
                ...
                if (!FileUtils.isFileExistsForFilesDir(fileName)) {
                    ...
                    new XPopup.Builder(context).dismissOnBackPressed(false)
                            .dismissOnTouchOutside(false).asCustom(dialog).show();
    ...
    private void gotoPanel(Context context, String rnDirName, DeviceInfoBean deviceInfoBean) {
        ...
        bundle.putString("id", deviceInfoBean.getGroupId());
        bundle.putString("name", deviceInfoBean.getName());
        // enum 设备类型
        bundle.putString("deviceType", getDeviceTypeData(deviceInfoBean.getProtocolType()));
        ...
        if (TextUtils.equals(deviceInfoBean.getProtocolType(), Constant.PROTOCOL_TYPE_FOR_SINGLE_BLUETOOTH)) {
            //单蓝牙
            networkType = 0;
            /// isSupportBLE = true;
        } else if (TextUtils.equals(deviceInfoBean.getProtocolType(), Constant.PROTOCOL_TYPE_FOR_WIFI_BLE)) {
            //wifi+蓝牙
            if (deviceInfoBean.getBindMode() == 1) {
                //绑定模式（0=网络，1=蓝牙）
                networkType = 1;
            } else {
                networkType = 2;
        ...
        if (deviceInfoBean.isCustomGroup()) {
            List<GroupPanelBean> subDevices = new ArrayList<>();
            ...
            if (groupDeviceInfo != null) {
                for (GroupDeviceItemBean item : groupDeviceInfo.getDeviceList()) {
                    GroupPanelBean groupPanelBean = new GroupPanelBean();
                    groupPanelBean.setDeviceId(item.getId());
                    subDevices.add(groupPanelBean);
                }
            }
            bundle.putString("subDevices", new Gson().toJson(subDevices));
        }
