...
public class FlowTagLayout extends ViewGroup {
    ...
    public static final int FLOW_TAG_SELECT = 3;
...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
            ...
            childView.setOnClickListener(v -> {
                if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                    if (mOnTagClickListener != null) {
                        isChildCheck = !childView.isSelected();
                        mOnTagClickListener.onItemClick(FlowTagLayout.this, childView, j);
                    }
                } else if (mTagCheckMode == FLOW_TAG_SELECT) {
    ...
    public void setOneCheck(int position) {
        if (position < 0) {
            return;
        }
        this.isChildCheck = true;
        for (int i = 0; i < mAdapter.getCount(); i++) {
            if (mCheckedTagArray.get(i) && position != i) {
                getChildAt(i).setSelected(false);
            } else {
                getChildAt(position).setSelected(true);
            }
    ...
    public void setSelectedData() {
        if (mOnTagSelectListener != null) {
            ...
            LgUtils.w("setSelectedData   list=" + list.size());
        }
    }
     ...
     * 将保存的数据重新添加进去
     */
    public void setAddSelectedStatus(int position, boolean isSelected) {
        mCheckedTagArray.put(position, isSelected);
    }
}
