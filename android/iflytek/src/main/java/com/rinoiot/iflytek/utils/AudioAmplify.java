...
public class AudioAmplify {
     ...
     */
    private static int originSample;
    /**
     * 放大的声道数量
     */
    private static int processChannels;
    /**
     * 1sample输入数据的字节数
     */
    private static int inputDataLengthPerSample;
    ...
    public static void amplifyAll(byte[] input, float multiple) {
        //每2字节循环一次，用局部变量提高效率
        for (int i = 0, inputLength = input.length; i < inputLength; i += 2) {
    ...
    private static void amplifyOneSample(byte[] input, int srcPos, float multiple) {
        //从short类型获取int类型的音频采样值(音量大小)
        int volume = byteToShort(input, srcPos);
        //计算调整后的音量
        volume = (int) (volume * multiple);
        if (volume < -32767) {
            //爆音的处理，16bit最小值-32767
            volume = -32767;
    ...
    public static void adjustValue(byte[] input, int value) {
        ...
        for (int i = 0, inputLength = input.length; i < inputLength; i += 2) {
            //从short类型获取int类型的音频采样值(音量大小)
            int volume = byteToShort(input, i);
            //计算调整后的音量
            volume = volume + value;
            if (volume < -32767) {
                //爆音的处理，16bit最小值-32767
