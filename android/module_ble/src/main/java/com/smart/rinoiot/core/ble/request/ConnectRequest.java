...
import com.smart.rinoiot.core.ble.BleStates;
...
import com.smart.rinoiot.core.ble.callback.wrapper.ConnectWrapperCallback;
...
import com.smart.rinoiot.core.ble.queue.retry.RetryDispatcher;
...
public class ConnectRequest<T extends BleDevice> implements ConnectWrapperCallback<T> {
     ...
     * @param device 设备对象
     */
    public void close(BleDevice device) {
        close(device, connectCallback);
    }
    /**
     *  带回调的断开且来释放资源
     *
     * @param device 设备对象
     */
    public void close(BleDevice device, BleConnectCallback<T> callback) {
        if (device != null) {
            connectCallback = callback;
            device.setAutoConnect(false);
            bleRequest.close(device.getBleAddress());
        }
    }
